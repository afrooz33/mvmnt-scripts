...
const GetTotalDonationsAndParticipantQuery = (query: MvmntDonationQueryDto): string => {
  let day_groupby = ''
  let day_select = ''
  let order_by = ''
  let left_join = ''
  ...
  const groupedQuery = `
    grouped_donation_participant AS (
      SELECT
        ...
        SUM(ud.amount) AS total_donations,
        COUNT(DISTINCT "ud"."userId") AS total_participants
      FROM
      ...
      WHERE
        udp.created BETWEEN '${start}' AND '${end}'
  ...
  return `WITH date_series AS (
    ...
    SELECT
      ...
      ${day_select}
      COALESCE(gdp.total_donations, 0) AS total_donations,
      COALESCE(gdp.total_participants, 0) AS total_participants
    FROM
      date_series ds
    LEFT JOIN
      grouped_donation_participant gdp
    ON
      EXTRACT(YEAR FROM ds.dates) = gdp.year
      AND EXTRACT(MONTH FROM ds.dates) = gdp.month
      ${left_join}
    ORDER BY
      year, month${order_by};`
}
...
export default async function (query: MvmntDonationQueryDto, isExport = false): Promise<any> {
  try {
    if (isExport) {
      const data: [] = await this.entityManager.query(GetTotalDonationsAndParticipantQuery(query))
    ...
    return await this.entityManager.query(GetTotalDonationsAndParticipantQuery(query))
  } catch (error) {
