...
import { GetNonprofitDataSql } from '@app/src/admin/re2/fundraiser/helpers'
import { GetDonationProjectDataSql } from '@app/src/admin/re2/fundraiser/helpers'
...
export default async function (query: QueryDto, userId: string): Promise<PaginateRO> {
  try {
    ...
    if (query?.gross_donation?.start && query?.gross_donation?.end) {
      results.condition.andWhere(`(data.amount - data.system_fees) BETWEEN :start AND :end`, {
    ...
    results.condition.select([
      ...
      `CASE
        WHEN "data"."reason" IN ('${DonationType.FUNDRAISER_FORM}', '${
            ...
            SELECT json_build_object(
              ...
              'nonprofits', (
                ${GetNonprofitDataSql(
                  ...
                  `"nonprofit"."re2ShopifyCartBannerSettingsId" = "cb"."id"`,
                )}
              ),
              'donation_projects', (
                ${GetDonationProjectDataSql(
                  're2_shopify_cart_banner_donation_projects',
                  `"donation_project"."re2ShopifyCartBannerSettingsId" = "cb"."id"`,
                )}
              )
            )
            ...
            SELECT json_build_object(
              ...
              'nonprofits', (
                ${GetNonprofitDataSql(
                  ...
                  `"nonprofit"."re2ShopifyCartDrawerSettingsId" = "cd"."id"`,
                )}
              ),
              'donation_projects', (
                ${GetDonationProjectDataSql(
                  're2_shopify_cart_drawer_donation_projects',
                  `"donation_project"."re2ShopifyCartDrawerSettingsId" = "cd"."id"`,
                )}
              )
            )
            ...
            SELECT json_build_object(
              ...
              'nonprofits', (
                ${GetNonprofitDataSql(
                  ...
                  `"nonprofit"."re2ShopifySalePortionSettingsId" = "sp"."id"`,
                )}
              ),
              'donation_projects', (
                ${GetDonationProjectDataSql(
                  're2_shopify_sale_portion_donation_projects',
                  `"donation_project"."re2ShopifySalePortionSettingsId" = "sp"."id"`,
                )}
              )
            )
