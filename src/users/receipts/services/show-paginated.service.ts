...
import { QueryPaginatedDto } from '@app/src/users/receipts/dto'
import { FundraiserType } from '@app/src/re2/fundraisers/enums'
import { DONATION_STATUS, DonationType } from '@app/src/donations/enums'

export default async function (query: QueryPaginatedDto, userId: string): Promise<PaginateRO> {
  try {
    ...
    results.condition.andWhere('"data"."status" = :donation_status', {
      donation_status:
        query?.is_history === 'Yes' ? DONATION_STATUS.SETTLED : DONATION_STATUS.COMPLETED,
    })

    if (query?.nonprofit && query?.is_history === 'No') {
      results.condition.andWhere('"donation_project"."userId" = :nonprofit', {
        nonprofit: query.nonprofit,
      })
    }

    if (query?.source_id && query?.is_history === 'Yes') {
