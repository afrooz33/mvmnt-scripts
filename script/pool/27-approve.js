/**
 * 27-approve.js
 * Pool Approve Script
 * Purpose: Approves spender to spend LP tokens
 */

const { ethers } = require('ethers');
require('dotenv').config({
    path: "../../.env"
});

async function runApprove(spender, amount, userPrivateKey = null) {
    try {
        console.log('🚀 Starting Approve Script...');
        
        // Setup provider and signer
        const provider = new ethers.JsonRpcProvider(process.env.BLOCKCHAIN_PROVIDER_RPCURL);
        const signerKey = userPrivateKey || process.env.USER_PRIVATE_KEY || process.env.ADMIN_KEY;
        const signer = new ethers.Wallet(signerKey, provider);
        
        console.log('📝 Caller Address:', await signer.getAddress());
        
        // Get contract address
        const poolAddress = process.env.POOL_CONTRACT_ADDRESS;
        if (!poolAddress) {
            throw new Error('❌ POOL_CONTRACT_ADDRESS not found in .env');
        }
        
        // Validate inputs
        if (!spender) {
            throw new Error('❌ Spender address is required');
        }
        if (!amount) {
            throw new Error('❌ Amount to approve is required');
        }
        
        // Convert inputs to proper formats
        if (!ethers.isAddress(spender)) {
            throw new Error('❌ Invalid spender address');
        }
        const amountWei = ethers.parseEther(amount.toString());
        
        console.log('🎯 Approve Parameters:');
        console.log('   - Spender address:', spender);
        console.log('   - Amount to approve:', amount);
        
        // Load contract
        const { PoolABI } = require('./abi/Pool.abi.js');
        const Pool = new ethers.Contract(poolAddress, PoolABI, signer);
        
        console.log('⏳ Sending transaction...');
        
        const tx = await Pool.approve(spender, amountWei);
        console.log('📤 Transaction Hash:', tx.hash);
        
        const receipt = await tx.wait();
        console.log('✅ Transaction confirmed in block:', receipt.blockNumber);
        console.log('⛽ Gas Used:', receipt.gasUsed.toString());
        
        
        
        console.log('🎉 Approve completed successfully!');
        
        return {
            success: true,
            transactionHash: tx.hash,
            gasUsed: receipt.gasUsed.toString(),
            0: 0,
            1: 1,
            poolAddress: poolAddress
        };
        
    } catch (error) {
        console.error('❌ Approve failed:', error.message);
        
        // Check for common errors
        if (error.message.includes('Unauthorized')) {
            return {
                success: false,
                error: 'Unauthorized access',
                code: 'UNAUTHORIZED'
            };
        }
        
        if (error.message.includes('paused')) {
            return {
                success: false,
                error: 'Pool is paused',
                code: 'POOL_PAUSED'
            };
        }
        
        return {
            success: false,
            error: error.message
        };
    }
}

// Run if called directly
if (require.main === module) {
    const [spender, amount, userPrivateKey] = process.argv.slice(2);
    
    if (!spender || !amount) {
        console.error('❌ Usage: node 27-approve.js "0x123..." 1000');
        process.exit(1);
    }
    
    runApprove(spender, amount, userPrivateKey)
        .then(result => {
            console.log('📊 Final Result:', result);
            process.exit(result.success ? 0 : 1);
        })
        .catch(error => {
            console.error('💥 Unexpected error:', error);
            process.exit(1);
        });
}

module.exports = { runApprove };