/**
 * 28-transfer.js
 * Pool Transfer Script
 * Purpose: Transfers LP tokens to another account
 */
...
async function runTransfer(to, amount, userPrivateKey = null) {
    try {
        ...
        const tx = await Pool.transfer(to, amountWei);
...
if (require.main === module) {
    const [to, amount, userPrivateKey] = process.argv.slice(2);
    
    if (!to || !amount) {
        console.error('❌ Usage: node 28-transfer.js "0x123..." 1000');
        process.exit(1);
    }
    
    runTransfer(to, amount, userPrivateKey)
        .then(result => {
