/**
 * 03-initializePool.js
 * LPManager InitializePool Script
 * Purpose: Initializes a pool with initial liquidity
 */
...
async function runInitializePool(brandId, tokenAmount, stableAmount, userPrivateKey = null) {
    try {
        console.log('🚀 Starting InitializePool Script...');
        ...
        console.log('🎯 InitializePool Parameters:');
        ...
        const tx = await LPManager.initializePool(brandIdWei, tokenAmountWei, stableAmountWei);
        ...
        console.log('🎉 InitializePool completed successfully!');
        ...
        console.error('❌ InitializePool failed:', error.message);
...
if (require.main === module) {
    const [brandId, tokenAmount, stableAmount, userPrivateKey] = process.argv.slice(2);
    
    if (!brandId || !tokenAmount || !stableAmount) {
        console.error('❌ Usage: node 03-initializePool.js 123 1000 1000');
        process.exit(1);
    }
    
    runInitializePool(brandId, tokenAmount, stableAmount, userPrivateKey)
        .then(result => {
...
module.exports = { runInitializePool };